...
# Ensure project root is in path before importing datasets (to avoid conflict with HuggingFace datasets package)
...
from datasets import eigenscape_dataset
from datasets import combined_spatial_dataset as data
from datasets.eigenscape_dataset import collate_fn
...
def _select_fixed_validation_segments(config, num_segments=5, duration=2.0):
    """Select fixed validation segments from datasets (eigenscape, starss, dcase, marco, locata).
    
    Uses a fixed random seed to ensure the same segments are selected every epoch.
    
    Returns:
        List of dicts with keys: 'audio_path', 'start_time', 'dataset_name', 'selected_channels'
    """
    ...
    # Collect files from Eigenscape
    try:
        eigenscape_base = Path(config.datasets.eigenscape_base_path)
        for folder_name in config.datasets.eigenscape_train_folders:
            folder_path = eigenscape_base / folder_name
            if folder_path.exists():
                wav_files = list(folder_path.glob("*.wav"))
                for wav_file in wav_files:
                    all_files.append(('eigenscape', wav_file))
    except Exception as e:
        logger.warning(f"Failed to load Eigenscape files: {e}")
    
    # Collect files from spatial datasets (STARSS, DCASE, LOCATA, MARCO) via CSV files
    try:
        import pandas as pd
        spatial_csvs = {
            'starss': config.datasets.get('starss_train_csv'),
            'dcase': config.datasets.get('dcase_train_csv'),
            'aug_locata': config.datasets.get('aug_locata_train_csv'),
            'aug_marco': config.datasets.get('aug_marco_train_csv')
        }
        
        for dataset_name, csv_path in spatial_csvs.items():
            if csv_path and Path(csv_path).exists():
                try:
                    df = pd.read_csv(csv_path, header=None)
                    for file_path in df[0].values:
        ...
        logger.warning(f"Failed to load spatial dataset files from CSV: {e}")
    
    if len(all_files) == 0:
        logger.warning("No audio files found for validation segments")
        return segments
    
    # Organize files by dataset
    files_by_dataset = {}
    for dataset_name, file_path in all_files:
        if dataset_name not in files_by_dataset:
            files_by_dataset[dataset_name] = []
        files_by_dataset[dataset_name].append(file_path)
    
    # Select 1 file from each target dataset (in order: eigenscape, starss, dcase, aug_locata, aug_marco)
    target_datasets = ['eigenscape', 'starss', 'dcase', 'aug_locata', 'aug_marco']
    selected_files = []
    min_rms_threshold = 0.01  # Minimum RMS to avoid silent files
    
    logger.info(f"Available datasets: {list(files_by_dataset.keys())}")
    
    for dataset_name in target_datasets:
        if dataset_name not in files_by_dataset or len(files_by_dataset[dataset_name]) == 0:
            logger.warning(f"⚠ Dataset {dataset_name} has no files available, skipping")
            continue
        
        # Try to find a non-silent file from this dataset
        max_attempts = min(100, len(files_by_dataset[dataset_name]))  # Try up to 100 files
        attempts = 0
        found_valid_file = False
        
        logger.info(f"Selecting from {dataset_name} ({len(files_by_dataset[dataset_name])} files available)")
        
        # Try to find a file with good RMS
        while attempts < max_attempts and not found_valid_file:
            # Randomly select a file
            audio_path = random.choice(files_by_dataset[dataset_name])
            
            try:
                # Quick check: load a small sample to check if it's silent
                test_duration = min(0.5, duration)  # Check first 0.5 seconds
                waveform_test, sr_test = librosa.load(
                    audio_path,
                    sr=config.model.sample_rate,
                    mono=True,  # Check mono for speed
                    offset=0,
                    duration=test_duration
                )
                
                # Calculate RMS
                rms = np.sqrt(np.mean(waveform_test ** 2))
                
                if rms >= min_rms_threshold:
                    # File has sufficient audio, check full duration
                    info = sf.info(audio_path)
                    file_duration = info.frames / info.samplerate
                    
                    if file_duration >= duration:
                        selected_files.append((dataset_name, audio_path))
                        found_valid_file = True
                        logger.info(f"✓ Selected from {dataset_name}: {audio_path.name} (RMS: {rms:.4f})")
            except Exception as e:
                logger.debug(f"Failed to check {audio_path}: {e}")
            
            attempts += 1
        
        # If no good file found, just pick any random file that's long enough
        if not found_valid_file:
            logger.info(f"Could not find file with RMS >= {min_rms_threshold} from {dataset_name}, selecting random file")
            for audio_path in random.sample(files_by_dataset[dataset_name], min(len(files_by_dataset[dataset_name]), 100)):
                try:
                    info = sf.info(audio_path)
                    file_duration = info.frames / info.samplerate
                    if file_duration >= duration:
                        selected_files.append((dataset_name, audio_path))
                        logger.info(f"✓ Selected from {dataset_name} (random): {audio_path.name}")
                        found_valid_file = True
                        break
                except Exception as e:
                    continue
            ...
            if not found_valid_file:
                logger.error(f"❌ Could not select any file from {dataset_name} - no files meet duration requirement")
    
    # We should have exactly 1 from each target dataset now
    # If we're missing any, that's an error (dataset had no files or all files too short)
    selected_datasets = {d for d, _ in selected_files}
    missing_datasets = [d for d in target_datasets if d not in selected_datasets]
    
    if missing_datasets:
        logger.warning(f"⚠ Could not select from datasets: {missing_datasets}")
    
    if len(selected_files) < num_segments:
        logger.warning(f"⚠ Only selected {len(selected_files)}/{num_segments} segments")
    
    # For each selected file, randomly select a 2-second segment
    for dataset_name, audio_path in selected_files:
        try:
            # Get file info
            info = sf.info(audio_path)
            file_duration = info.frames / info.samplerate
            
            # Ensure file is long enough for 2-second segment
            if file_duration < duration:
                continue
            
            # Random start time (leave duration at end)
            max_start_time = max(0, file_duration - duration)
            start_time = random.uniform(0, max_start_time)
            
            # Verify the selected segment is not silent
            segment_found = False
            try:
                waveform_check, _ = librosa.load(
                    audio_path,
                    sr=config.model.sample_rate,
                    mono=True,
                    offset=start_time,
                ...
                rms_segment = np.sqrt(np.mean(waveform_check ** 2))
                if rms_segment >= min_rms_threshold:
                    segment_found = True
                else:
                    # Segment is too silent, try a different start time
                    # Try up to 10 different start times
                    ...
                    # If still not found, try a different file from the same dataset
                    if not segment_found:
                        logger.warning(f"Could not find non-silent segment in {audio_path.name}, trying different file from {dataset_name}")
                        # Try up to 3 different files from the same dataset
                        alternative_files = [f for f in files_by_dataset[dataset_name] if f != audio_path]
                        for alt_file in random.sample(alternative_files, min(3, len(alternative_files))):
                            try:
                                alt_info = sf.info(alt_file)
                                alt_duration = alt_info.frames / alt_info.samplerate
                                if alt_duration < duration:
                                    continue
                                
                                alt_max_start = max(0, alt_duration - duration)
                                # Try 5 start times for this alternative file
                                for _ in range(5):
                                    alt_start = random.uniform(0, alt_max_start)
                                    alt_waveform, _ = librosa.load(
                                        alt_file,
                                        sr=config.model.sample_rate,
                                        mono=True,
                                        offset=alt_start,
                                    ...
                                    alt_rms = np.sqrt(np.mean(alt_waveform ** 2))
                                    if alt_rms >= min_rms_threshold:
                                        # Found a good segment in alternative file
                                        audio_path = alt_file
                                        start_time = alt_start
                                        segment_found = True
                                        logger.info(f"✓ Found non-silent segment in alternative file: {alt_file.name}")
                                        break
                                
                                if segment_found:
                                    break
                            except Exception as e:
                                logger.debug(f"Failed to check alternative file {alt_file}: {e}")
                                continue
                        
                        if not segment_found:
                            # Last resort: just use any segment from the original file
                            logger.warning(f"⚠ Using {audio_path.name} despite silent segments - no better alternative found")
                            start_time = random.uniform(0, max_start_time)
                            segment_found = True
            except Exception as e:
                logger.debug(f"Could not verify segment for {audio_path}: {e}")
                # Continue anyway if verification fails
                segment_found = True  # Assume it's okay if we can't verify
            
            # Only add segment if we found a valid one
            if not segment_found:
                logger.warning(f"⚠ Skipping {dataset_name} - could not find valid segment")
                continue
            
            # For multi-channel files, select channels (for Eigenscape: 32 channels -> 2 channels)
            selected_channels = None
            if dataset_name == 'eigenscape':
                # Eigenscape has 32 channels, select 2 random channels
                selected_channels = random.sample(range(32), config.datasets.num_channels_to_load)
            else:
                # For other datasets, use first 2 channels if available
                # We'll determine this when loading
                selected_channels = list(range(min(2, config.model.channels)))
    ...
    # Log final selection
    final_datasets = [d for d, _ in selected_files]
    dataset_counts = {d: final_datasets.count(d) for d in set(final_datasets)}
    logger.info(f"Selected {len(segments)} fixed validation segments from datasets: {dataset_counts}")
...
def _upload_validation_audio_samples(epoch, model, config, wandb_logger):
    ...
    Uses 5 fixed 2-second segments randomly selected from datasets (eigenscape, starss, dcase, marco, locata),
    reconstructs at different bandwidths, and uploads as WandB tables and artifacts.
    Same segments are used every epoch (deterministic).
    """
    logger.info("Generating validation audio samples for WandB upload...")
    
    # Select fixed validation segments (same every epoch)
    validation_segments = _select_fixed_validation_segments(config, num_segments=5, duration=2.0)
    
    if len(validation_segments) == 0:
        logger.warning("No validation segments available. Skipping audio upload.")
        return
    
    logger.info(f"Using {len(validation_segments)} fixed validation segments")
    ...
    try:
        ...
        listening_table = wandb.Table(columns=["dataset", "bandwidth", "type", "audio", "si_snr"])
        
        from utils import convert_audio
        import librosa
        import soundfile as sf
        
        for seg_idx, segment in enumerate(validation_segments):
            audio_path = segment['audio_path']
            start_time = segment['start_time']
            dataset_name = segment['dataset_name']
            selected_channels = segment['selected_channels']
            
            logger.info(f"Processing segment {seg_idx+1}/{len(validation_segments)}: {dataset_name} - {audio_path.name}")
            ...
            try:
                ...
                # Convert to tensor
                waveform = torch.as_tensor(waveform)  # [C, T]
                
                # Select channels if needed
                if len(waveform.shape) > 1 and waveform.shape[0] > config.model.channels:
                    if selected_channels is not None:
                        waveform = waveform[selected_channels, :]  # [num_channels, T]
                    else:
                        # Randomly select channels
                        waveform = waveform[:config.model.channels, :]
                elif len(waveform.shape) == 1:
                    # Mono audio, expand to channels
                    waveform = waveform.unsqueeze(0)  # [1, T]
                    if config.model.channels > 1:
                        waveform = waveform.expand(config.model.channels, -1)
                
                # Convert to model format
                waveform = convert_audio(waveform, sample_rate, config.model.sample_rate, config.model.channels)
                # Add batch dimension: [C, T] -> [1, C, T]
                gt_audio = waveform.unsqueeze(0)
                ...
                segment_id = f"{dataset_name}_{seg_idx+1}"
                
                # Save ground truth audio (move to CPU first)
                gt_path = temp_dir / f"{segment_id}_gt.wav"
                ...
                for bandwidth in sample_bandwidths:
                    ...
                    recon_path = temp_dir / f"{segment_id}_bw_{bandwidth}.wav"
                    ...
                    try:
                        ...
                        si_snr_value = None
                        logger.warning(f"Failed to calculate SI-SNR for {segment_id} at {bandwidth} kbps: {e}")
                ...
                logger.warning(f"Failed to process segment {seg_idx+1}: {e}")
                continue
...
def train(config):
    ...
    # Set up logging
    file_handler = logging.FileHandler(f"{config.checkpoint.save_folder}/train_spatial_consistency_bs{config.datasets.batch_size}_lr{config.optimization.lr}.log")
    ...
    if config.get('wandb', {}).get('enabled', True):
        try:
            wandb.init(
                project=config.get('wandb', {}).get('project', 'eigenscape-encodec-spatial-consistency'),
                name=config.get('wandb', {}).get('name', f'spatial_consistency_bs{config.datasets.batch_size}_lr{config.optimization.lr}'),
                config=dict(config),
    ...
    # Set up datasets - use combined Eigenscape + spatial datasets by default
    # This combines EigenscapeDataset (32-channel -> 2 random channels) 
    # with MultiDataset (spatial audio datasets with 2-4 channels -> 2 random channels)
    trainset = data.CombinedSpatialDataset(
        config=config, 
        mode='train',
        use_eigenscape=True,  # Include Eigenscape dataset
        use_spatial=True      # Include spatial audio datasets (STARSS, DCASE, LOCATA, MARCO)
    )
    ...
    for epoch in range(start_epoch, config.common.max_epoch + 1):
        ...
        if epoch % config.common.val_interval == 0 and epoch > 0:
            ...
            # Use combined dataset for validation too
            valset = data.CombinedSpatialDataset(
                config=config, 
                mode='val',
                use_eigenscape=True,
                use_spatial=True
        ...
        if epoch % config.common.save_interval == 0:
            ...
            if wandb_logger and upload_artifacts:
                try:
                    artifact = wandb.Artifact(f'spatial_consistency_model_epoch_{epoch}', type='model')
...
@hydra.main(config_path='config', config_name='config_spatial_consistency')
def main(config):
    # Disable cudnn
