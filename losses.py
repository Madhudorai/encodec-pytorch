...
def total_loss(fmap_real, logits_fake, fmap_fake, input_wav, output_wav, sample_rate=24000):
    ...
    for tt1 in range(len(fmap_real)): # len(fmap_real) = 3
        ...
        for tt2 in range(len(fmap_real[tt1])): # len(fmap_real[tt1]) = 5
            ...
            mean_abs = torch.mean(torch.abs(fmap_real[tt1][tt2]))
            # Add small epsilon to prevent division by zero (could cause NaN)
            mean_abs = mean_abs + 1e-8
            l_feat = l_feat + l1Loss(fmap_real[tt1][tt2], fmap_fake[tt1][tt2]) / mean_abs
